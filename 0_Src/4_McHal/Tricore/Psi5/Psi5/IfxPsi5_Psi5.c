 ...
 * \return None
 */
static void IfxPsi5_Psi5_enableModule(Ifx_PSI5 *psi5);

/** \brief Reset PSI5 kernel
 * \param psi5 pointer to the base of PSI5 registers
 * \return None
 */
static void IfxPsi5_Psi5_resetModule(Ifx_PSI5 *psi5);
 ...
 */
static uint32 IfxPsi5_Psi5_getFracDivClock(Ifx_PSI5 *psi5);
 ...
 */
static uint32 IfxPsi5_Psi5_initializeClock(Ifx_PSI5 *psi5, const IfxPsi5_Psi5_Clock *clock);
...
boolean IfxPsi5_Psi5_initChannel(IfxPsi5_Psi5_Channel *channel, const IfxPsi5_Psi5_ChannelConfig *config)
{
    ...
    if (pins != NULL_PTR)
    {
        ...
        if (rx != NULL_PTR)
        {
            IfxPsi5_initRxPin(rx, pins->inMode);
        }
...
{
    channel->channel->SDRL.U = data & 0xFFFFFFFF;
    channel->channel->SDRH.U = (data >> 32) & 0xFFFFFFFF;
